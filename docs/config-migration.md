...
程序可以直接加载 Python v11–v15 配置执行任务，但这种兼容加载只在内存中转换，并将配置源标记为只读。国内签到或米游社任务刷新 `cookie_token` 后会继续完成本轮任务，但不会按新版 `accounts` 结构回写旧文件。需要持久保存刷新凭据时，应先使用 `migrate-config` 生成新版 YAML，再改用迁移后的文件运行。
...
迁移会保留旧配置中的 `name`、`model`、`id` 和 `fp`。缺失的 `name` 和 `model` 分别使用 `Xiaomi MI 6` 和 `Mi 6`；缺失或留空的 `id` 会在运行时根据账号 Cookie 使用 UUID v3 确定性地自动生成。非空 `fp` 会用于米游社 App 类社区请求的设备指纹请求头；留空时不会发送该请求头。

旧版 `games.cn` 与 `games.os` 不再合并：国内游戏、User-Agent 和 UID 黑名单迁移到国内签到配置；国际服独立 Cookie、语言和游戏迁移到 `hoyolab`。原 Python 国际服签到没有按 UID 使用 `black_list`，因此国际服黑名单会产生明确警告而不会被伪装成已迁移。
