...
本项目使用 GitHub Actions 完成 Rust 格式检查、Clippy、测试、Linux/Windows 构建、Nix 构建、TLS 握手和多架构容器验证。实现状态与仍需人工验收的项目见 [重构实施文档](RUST_REWRITE_GUIDE.md#当前实现进度)，后续面向普通用户的改进见 [用户体验优化方案](docs/用户体验优化方案.md)，依赖与体积优化记录见 [构建体积优化说明](docs/size-optimization.md)。
