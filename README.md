...
[下载最新版本](https://github.com/bcmdy/mihoyo-bbs-tools-rs/releases/latest) · [使用说明（含快速开始与全部配置）](docs/使用说明.md)

## 主要功能

- 国内游戏签到与 HoYoLAB 国际服游戏签到。
- 米游社社区签到可独立选择板块；阅读、点赞、取消点赞和分享均可单独开关。每项任务由账号配置执行首轮，再按固定任务 ID 与实时米游币差值独立复查、按需重试。
- 国内云原神、云绝区零和国际服云原神签到。
...
- `使用说明.md`，包含快速开始、全部配置、通知、故障处理与安全要求；
...
更详细的首次配置、终端用法和故障排查步骤见 [使用说明](docs/使用说明.md)。
...
数字菜单可以配置全部常用节点，包括多账号、国内与 HoYoLAB 游戏、社区板块、云游戏、角色黑名单、设备 ID/FP、代理、验证码、日志、调度及全部通知渠道。所有菜单修改先暂存；退出时显示不含 Secret 的摘要，经确认后才原子替换真实配置并创建备份。熟悉 YAML 的用户可参考 [使用说明中的全字段说明](docs/使用说明.md#13-configyaml-全字段中文说明) 或运行 `config edit`。

用户文档统一收录在 [使用说明](docs/使用说明.md)，包括快速开始、全部 YAML 字段、旧配置迁移、凭据安全、日志、通知、自动运行和常见问题。
...
本项目使用 GitHub Actions 完成 Rust 格式检查、Clippy、测试、Linux/Windows 构建、Nix 构建、TLS 握手和多架构容器验证。已完成方案和历史优化记录由 Git 提交历史保存，不再作为用户文档分发。
