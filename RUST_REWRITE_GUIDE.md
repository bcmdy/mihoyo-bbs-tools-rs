...
| 阶段 7：推送 | 大部分完成 | 已实现 Telegram、Webhook、PushPlus、Server酱、企业微信、钉钉、飞书、Bark、Gotify、Discord、WxPusher、SMTP 等通知渠道；Windows 本地通知待补充。 |
| 阶段 8：完整运行、Docker 与迁移 | 部分完成 | `run`、`schedule` 常驻间隔调度、GitHub 定时任务、旧配置迁移、Docker、Release 工作流及 Linux/Windows 发布附件已实现；Docker 远程运行验收仍缺失。 |
...
- [x] 国内与 HoYoLAB 签到提交后再次确认服务端状态，只重试复查后仍未完成的角色，并显示当天奖励详情。
...
- [x] 云游戏可以按区域和单游戏单独启用或关闭。
- [x] Web 活动可以单独启用或关闭并产生明确结果。
- [x] 推送汇总不会泄露敏感信息。
