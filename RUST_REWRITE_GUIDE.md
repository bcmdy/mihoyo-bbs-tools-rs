...
- [ ] 创建 `mihoyo-bbs-tools-rs` 仓库和同名项目目录。
- [ ] 初始化 Cargo 包名为 `mihoyo-bbs-tools` 的 Rust 2024 二进制项目。
- [ ] 添加中文 README 和许可证。
- [ ] 添加 `ci.yml`。
- [ ] 实现 `version` 命令。
- [ ] 推送并确保 Linux/Windows CI 通过。
- [ ] 添加 YAML 配置模型和示例。
- [ ] 实现 `validate-config`。
- [ ] 添加环境变量替换和 Secret 脱敏。
- [ ] 实现统一 HTTP 客户端。
- [ ] 使用 Mock 服务测试 HTTP 行为。
- [ ] 实现 Cookie 解析和签名工具。
- [ ] 实现一个国内游戏的状态查询。
- [ ] 手动运行真实接口测试。
- [ ] 实现该游戏的实际签到。
