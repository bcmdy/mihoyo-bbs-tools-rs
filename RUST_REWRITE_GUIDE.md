...
| 阶段 1：配置、日志与 HTTP | 部分完成 | YAML、多账号、环境变量替换、校验、代理、重试和 Secret 脱敏已实现；运行时日志级别尚未接入，统一 HTTP 重试/超时合约测试仍需补齐。 |
| 阶段 2：账号与认证 | 部分完成 | Cookie、SToken、登录票据刷新、设备 ID、DS 与 MD5 已实现；SHA256/HMAC 工具尚未补齐。 |
| 阶段 3：国内游戏签到 | 代码完成，待真实验收 | 六款国服游戏、角色查询、状态查询、签到与错误分类已实现；真实账号仍需人工验证。 |
| 阶段 4：HoYoLAB 国际服签到 | 代码完成，待真实验收 | 五款国际服游戏、多账号、独立接口与统一报告已实现；真实账号仍需人工验证。 |
| 阶段 5：米游社社区任务和验证码 | 部分完成 | 社区签到、阅读、点赞、取消点赞、分享、米游币汇总和验证码阻断已实现；验证码服务客户端/求解尚未实现。 |
| 阶段 6：云游戏与 Web 活动 | 未开始 | 国内/国际云游戏和 Web 活动尚未迁移。 |
| 阶段 7：推送 | 未开始 | Telegram、企业微信、钉钉等通知提供商尚未实现。 |
| 阶段 8：完整运行、Docker 与迁移 | 部分完成 | `run`、旧配置迁移、Docker 和 Release 工作流已实现；定时工作流缺失，Release 发布作业仍需修复并验证附件。 |
...
3. 实现验证码服务客户端和求解结果提交；当前仅能识别验证码并停止高风险请求。
4. 迁移国内/国际云游戏和 Web 活动，并避免配置开启后被静默忽略。
5. 实现通知提供商和统一推送汇总，优先 Telegram、企业微信与钉钉。
6. 增加 GitHub Actions 定时运行，并实际消费时区和随机延迟配置。
7. 修复 Release 发布作业；当前构建和 Artifact 上传成功，但 GitHub Release 没有可下载附件。
8. 增加 Docker 构建/运行工作流，验证最终镜像可在无 Rust 环境运行。
9. 将认证刷新接入签到和米游社运行器；当前刷新客户端只有离线测试，业务流程尚未调用。
10. 补齐 SHA256/HMAC、HTTP 响应体大小限制、重试/超时合约测试和多账号故障隔离测试。
11. 增加 Secret 历史扫描、依赖许可证清单及 arm64/armv7 构建验证。
